...
export default {
  ...
  mounted() {
    ...
    this.debouncedValidation = _.debounce(this.validateQuery, 500);
  },
  methods: {
    onFocusSearchField() {
    ...
    onInputSearchField(query) {
      ...
      this.debouncedValidation(query);
    },
    validateQuery(query) {
      if (!query) {
      ...
      const items = this.filterTableItems(query);
      if (items.length) {
    ...
    filterTableItems(query = "") {
      return initialData.data.filter(
        item => ~item.entity.toLowerCase().search(query.toLowerCase())
      );
    },
    hideDynamicOptions() {
