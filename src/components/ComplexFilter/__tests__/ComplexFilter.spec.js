...
describe('ComplexFilter.vue', () => {
  const createWrapper = () => {
    ...
    wrapper.vm.updateData = () => {
      ...
      wrapper.vm.dOptions.update(null, wrapper.vm.tableData)
    }
    wrapper.vm.forwardFilter(FilterState.initialState)
  ...
  const changeState = (wrapper, option) => {
    ...
    wrapper.vm.query.addOption(option, wrapper.vm.state)

    wrapper.vm.updateData = () => {
      wrapper.vm.applyData(mockData.data)
      wrapper.vm.dOptions.update(null, wrapper.vm.tableData)
    }
    wrapper.vm.forwardFilter(wrapper.vm.state)
  }

  const clickOnDynamicOption = (wrapper, option) => {
    wrapper.vm.updateData = () => {
      wrapper.vm.applyData(mockData.data)
      const options = wrapper.vm.getDynamicOptionsByState(wrapper.vm.state)
      wrapper.vm.dOptions.update(options, wrapper.vm.tableData)
    }
  ...
  const validQueries = [
    ...
    // 'Desserts: name '
    // 'Desserts: name != ',
    // 'Desserts: name != Eclair ',
    // 'Desserts: name != Eclair EMPTY ',
    // 'Desserts: name != Eclair EMPTY fat ',
    // 'Desserts: name != Eclair EMPTY fat != ',
    // 'Desserts: name != Eclair EMPTY fat != 16 '
  ]
  ...
  test('should modify tableItems on input event', () => {
  ...
  // })
  //
  // test('parse and validate queries', () => {
  //   const wrapper = createWrapper()
  //   validQueries.forEach((str) => {
  //     wrapper.find(SearchField).vm.$emit('input-search-field', str)
  //     expect(wrapper.vm.query.isValid).toBeTruthy()
  //   })
  //   // invalidQueries.forEach((str) => {
  //   //   wrapper.find(SearchField).vm.$emit('input-search-field', str)
  //   //   expect(wrapper.vm.query.isValid).toBeFalsy()
  //   // })
